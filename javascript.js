...
function init() {
	...
	// document.onkeydown = handleKey; ding,chengliang
...
function select (i) { // i 0-xx
	...
	items[i].sxsel=true;
	items[i].style.background="#d1fef5";
	items[i].lastChild.style.color="#00a0b0";
	...
	if (acdiv==0) {
		...
		if (i<k_action.length) {
			switch (k_action[i]) {
				...
				case "edit":	ac_edit();return false;
				case "newfile":	ac_newfile();return false;
...
function update_tb() {
	...
	if (sel) {
		...
		document.getElementById("icedit").src="img/tool_edit.gif";		
		document.getElementById("iczip").src="img/tool_zip16.gif";
	} else {
		...
		document.getElementById("icedit").src="img/tool_edit_bw.gif";
		document.getElementById("iczip").src="img/tool_zip16.gif";
	}
...
function ac_edit() {
	...
	if (sel>1) 
	{ alert("Too many file selected!"); return;}
  document.f.arg.value = getselection();
	document.f.action.value = "edit";
	subfrm();
 
}
function ac_newfile() {
	var i;
	i=prompt ("New File","NewFile.txt");
	if (i ) {
		document.f.action.value="newfile";
...
function ac_copy(cut) {
	...
	update_tb();
	update_cut();
  showNotify(document.f.cb_files.value + '<br> is in the clipboard, for ' + (cut?'cut':'copy') );
}
function ac_paste() {
	if (!cb_files) return;
...
function ac_rename() {
	...
	f.style.border='1px solid #aaaa33';
	f.style.background='#ffffff';
	f.style.width='200px';
	f.select();
	f.focus();
	f.onblur=do_rename;
	f.onkeydown=do_renamekeydown;
}
function do_rename() {
	var i;
	ca_rename();
...
function ac_phpinfo() { 
 document.f.action.value="phpinfo"; 
...
function ac_zip() {
	if (!(document.f.arg.value=getselection())) return;
	if (sel>1) msg=" these "+sel+" items?"
	else msg=" '"+document.f.arg.value+"' "
		i=prompt ("input Zip file name for\r\n" + msg , "myzipfile");
	if (i) {
		document.f.action.value="zip";
		document.f.zip_filename.value= i ;
		subfrm();
	}
	docfocus();
}
...
function ac_extract() {
	zipfile = items[curpos-1].title;
	if (!zipfile.match(/\.zip$/i)) return;
	
	i=prompt ("input extract Path for " + zipfile , curdir);
	if (i) {
		document.f.arg.value= i;
		document.f.zip_filename.value= zipfile;
...
function ac_savefile() {
    //fname = document.getElementById("ccbut").value;
   // xstr=  'code=' + encodeURIComponent(document.forms['editfrm'].code.value);
    xstr =  "code=" + encodeURIComponent(document.getElementById("editcode").value);
     
    //fname =  document.forms['editfrm'].fname.value;  
    fname =  document.getElementById("editfname").value; 
  	xmlHttp.open("POST", "index.php?action=savefileonly&fname=" + fname, true);
    xmlHttp.onreadystatechange = ac_savefile1;
...
function ac_savefile1() {
	if (xmlHttp.readyState == 4) {
		showNotify(xmlHttp.responseText);
		document.getElementById("savereturn").innerText = xmlHttp.responseText;
	}
}
...
{	
	new Dialog(msg,{time:2000, bgColor:'#000000'}).show();
	 	 
}

function shownError(msg)
{		 
	new Dialog(msg,{bgColor:'#dd0000', title:'Error'}).show();
