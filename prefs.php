...
function getserverroot()
{
	...
	if( !isset($_SERVER['DOCUMENT_ROOT']))
	{
		...
		if(isset($_SERVER['SCRIPT_FILENAME']))  
		  ...
		  $fullpath = $_SERVER['ORIG_PATH_TRANSLATED'];  
		  
		$_SERVER['DOCUMENT_ROOT'] = substr($fullpath,0, strlen($fullpath)-strlen($self))  ;
...
<div class="tab-pane">
	...
	<table border=0 cellpadding=2 cellspacing=0 width=100%>
		<tr><td>Root Path</td><td><input size=42 name="cfg[root_path]" value="<?php echo htmlspecialchars($cfg["root_path"]); ?>" >
			<a href=# onclick="document.all['defaultRoot'].style.display = 'block';" title='get default path'><b>??</b></a>		 
		</td>
		</tr>
		<tr id='defaultRoot' style='display:none' >
			<td align=right>curdir:<br>
				server root:</td>
		  <td><input size=40 value='<? echo preg_replace("/\\\\/","/",getcwd()); ?> '><br>
			  <input size=40 value='<? echo getserverroot(); ?> '>
			</td>
	  </tr>
